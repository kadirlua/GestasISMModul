...
<title>GESTAŞ-VIYA-TAMİM LİSTESİ</title>
    	...
    	<table id="customers">
			...
			<tr>
			 ...
			 <?php
			 	...
			 	$sql_query = "SELECT * FROM circular_list,department WHERE department.department_id=circular_list.department_id AND form_type=0 Order By circular_list_id DESC LIMIT 25;";
			 	if ($_POST)
			 	{
			 		...
			 		switch ($search_type) {
					    case "department_id":
					    	$sql_query = "SELECT * FROM circular_list,department WHERE department.department_name LIKE '%".$search_value."%' AND department.department_id = circular_list.department_id AND form_type=0 Order By circular_list.circular_list_id DESC";
					        break;
					    case "who_created":
					        $sql_query = "SELECT * FROM circular_list,users,department WHERE users.name LIKE '%".$search_value."%' AND users.user_id = circular_list.who_created AND department.department_id=circular_list.department_id AND form_type=0 Order By circular_list.circular_list_id DESC";
					        break;
					    case "employee_circular_id":
					        $sql_query = "SELECT * FROM circular_list,employee_circular_list,users,department WHERE users.name LIKE '%".$search_value."%' AND users.user_id = employee_circular_list.employee_circular_id AND employee_circular_list.circular_list_id = circular_list.circular_list_id AND department.department_id=circular_list.department_id AND form_type=0 Order By circular_list.circular_list_id DESC";
					        break;
					    default:
					        $sql_query = "SELECT * FROM circular_list,department WHERE ".$search_type." LIKE '%".$search_value."%' AND department.department_id=circular_list.department_id AND form_type=0 Order By circular_list_id DESC";
					}
			 	}
